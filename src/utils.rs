...
// todo
// pub fn from_base64_string(b64_string: &str) -> Result<String> {
//     let mut buf = [0u8; BASE64_BUFFER_SIZE];

//     let string = Base64::decode(b64_string, &mut buf)
...
// pub fn to_base64_string(string: &str) -> Result<String> {
//     let mut buf = [0u8; BASE64_BUFFER_SIZE];

//     let b64_string = Base64::encode(string.as_bytes(), &mut buf)
//         .map_err(|err| Error::new(ErrorKind::Todo, err.to_string()))?
//         .to_string();
...
pub fn from_base64(bytes: &[u8]) -> Result<Vec<u8>> {
    let mut buf = [0u8; BASE64_BUFFER_SIZE];

    let bytes = Base64::decode(bytes, &mut buf)
        .map_err(|err| Error::new(ErrorKind::Todo, err.to_string()))?;

    Ok(bytes.to_vec())
}

pub fn to_base64(bytes: &[u8]) -> Result<Vec<u8>> {
    let mut buf = [0u8; BASE64_BUFFER_SIZE];

    let b64_bytes = Base64::encode(bytes, &mut buf)
        .map_err(|err| Error::new(ErrorKind::Todo, err.to_string()))?
        .as_bytes()
        .to_vec();
